<template><div><h2 id="缺页中断" tabindex="-1"><a class="header-anchor" href="#缺页中断" aria-hidden="true">#</a> 缺页中断</h2>
...
<ol>
<li>
<p>最佳⻚⾯置换算法（<em>OPT</em>）</p>
</li>
<li>
<p>先进先出置换算法（<em>FIFO</em>）最近最久未使⽤的置换算法（<em>LRU</em>）</p>
</li>
<li>
<p>时钟⻚⾯置换算法（<em>Lock</em>）</p>
</li>
<li>
<p>最不常⽤置换算法（<em>LFU</em>）</p>
</li>
</ol>
<h3 id="最佳⻚面置换算法opt" tabindex="-1"><a class="header-anchor" href="#最佳⻚面置换算法opt" aria-hidden="true">#</a> 最佳⻚⾯置换算法OPT</h3>
<p>最佳⻚⾯置换算法基本思路是，<strong>置换在「未来」最⻓时间不访问的⻚⾯</strong>。</p>
<p>这很理想，但是实际系统中⽆法实现，因为程序访问⻚⾯时是动态的，我们是⽆法预知每个⻚⾯在「下⼀次」访问前的等待时间。</p>
<p>所以，最佳⻚⾯置换算法作⽤是为了衡量你的算法的效率，你的算法效率越接近该算法的效率，那么说明你的算法是⾼效的。</p>
<h3 id="先进先出置换算法fifo" tabindex="-1"><a class="header-anchor" href="#先进先出置换算法fifo" aria-hidden="true">#</a> 先进先出置换算法FIFO</h3>
<p>既然我们⽆法预知⻚⾯在下⼀次访问前所需的等待时间，那我们可以<strong>选择在内存驻留时间很⻓的⻚⾯进⾏中置换</strong>，这个就是「先进先出置换」算法的思想。</p>
<h3 id="最近最少使用的置换算法lru" tabindex="-1"><a class="header-anchor" href="#最近最少使用的置换算法lru" aria-hidden="true">#</a> 最近最少使⽤的置换算法LRU</h3>
<p>最近最少使⽤（<em>LRU</em>）的置换算法的基本思路是，发⽣缺⻚时，选择最⻓时间没有被访问的⻚⾯进⾏置换，也就是说，<strong>该算法假设已经很久没有使⽤的⻚⾯很有可能在未来较⻓的⼀段时间内仍然不会被使⽤</strong>。</p>
<p>这种算法近似最优置换算法，最优置换算法是通过「未来」的使⽤情况来推测要淘汰的⻚⾯，⽽ LRU 则是通过「历史」的使⽤情况来推测要淘汰的⻚⾯。</p>
<p>虽然 LRU 在理论上是可以实现的，但代价很⾼。为了完全实现 LRU，需要在内存中维护⼀个所有⻚⾯的链表，最近最多使⽤的⻚⾯在表头，最近最少使⽤的⻚⾯在表尾。
困难的是，在每次访问内存时都必须要更新「整个链表」。在链表中找到⼀个⻚⾯，删除它，然后把它移动到表头是⼀个⾮常费时的操作。</p>
<blockquote>
<p>所以，LRU 虽然看上去不错，但是由于开销⽐较⼤，实际应⽤中⽐较少使⽤。</p>
</blockquote>
<h3 id="时钟⻚面置换算法——clock算法" tabindex="-1"><a class="header-anchor" href="#时钟⻚面置换算法——clock算法" aria-hidden="true">#</a> 时钟⻚⾯置换算法——Clock算法</h3>
<p>简单的CLOCK算法是给每一帧关联一个附加位，称为使用位。</p>
...
<h3 id="改进型的-clock-算法" tabindex="-1"><a class="header-anchor" href="#改进型的-clock-算法" aria-hidden="true">#</a> 改进型的 clock 算法</h3>
<p>CLOCK算法的性能比较接近LRU，而通过增加使用的位数目，可以使得CLOCK算法更加高效。在使用位的基础上再增加一个修改位，则得到改进型的CLOCK置换算法。</p>
...
<h3 id="最不常用页面置换算法" tabindex="-1"><a class="header-anchor" href="#最不常用页面置换算法" aria-hidden="true">#</a> 最不常⽤页面置换算法</h3>
<p>当发⽣缺⻚中断时，选择「访问次数」最少的那个⻚⾯，并将其淘汰。
它的实现⽅式是，对每个⻚⾯设置⼀个「访问计数器」，每当⼀个⻚⾯被访问时，该⻚⾯的访问计数器就累加 1。在发⽣缺⻚中断时，淘汰计数器值最⼩的那个⻚⾯。</p>
<h1 id="页面抖动-颠簸-和工作集-驻留集" tabindex="-1"><a class="header-anchor" href="#页面抖动-颠簸-和工作集-驻留集" aria-hidden="true">#</a> 页面抖动(颠簸)和工作集(驻留集)</h1>
<h2 id="页面抖动-颠簸" tabindex="-1"><a class="header-anchor" href="#页面抖动-颠簸" aria-hidden="true">#</a> 页面抖动（颠簸）</h2>
<p>在页面置换过程中的一种最糟糕的情形是，刚刚换出的页面马上又要换入主存，刚刚换入的页面马上就要换出主存，这种频繁的页面调度行为称为抖动，或颠簸。</p>
...
<h2 id="工作集-驻留集" tabindex="-1"><a class="header-anchor" href="#工作集-驻留集" aria-hidden="true">#</a> 工作集（驻留集）</h2>
<p>工作集（或驻留集）是指在某段时间间隔内，进程要访问的页面集合。</p>
