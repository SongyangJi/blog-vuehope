<template><div><p>当希望MySQL能够以更高的性能运行查询时，最好的办法就是弄清楚MySQL是如何优化和执行查询的。一旦理解了这一点，很多查询优化工作实际上就是遵循一些原则能够按照预想的合理的方式运行。
 如上图所示，当向MySQL发送一个请求的时候，MySQL到底做了什么。</p>
<figure><img src="https://img-blog.csdnimg.cn/20191013222917867.jpeg?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L2Zpc2hfc3R1ZHlfY3Nkbg==,size_16,color_FFFFFF,t_70" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<ol>
<li>客户端发送一条查询给服务器。</li>
...
<h2 id="查询缓存" tabindex="-1"><a class="header-anchor" href="#查询缓存" aria-hidden="true">#</a> <strong>查询缓存</strong></h2>
...
 事实则是，如果查询语句中包含任何的不确定的函数，那么其查询结果不会被缓存，因为查询缓存中也无法找到对应的缓存结果。</p>
<h2 id="解析和预处理" tabindex="-1"><a class="header-anchor" href="#解析和预处理" aria-hidden="true">#</a> <strong>解析和预处理</strong></h2>
...
<h2 id="查询优化器" tabindex="-1"><a class="header-anchor" href="#查询优化器" aria-hidden="true">#</a> <strong>查询优化器</strong></h2>
...
<figure><img src="https://imgconvert.csdnimg.cn/aHR0cHM6Ly9zZWdtZW50ZmF1bHQuY29tL2ltZy9iVmJmZGRR?x-oss-process=image/format,png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
...
<h2 id="查询执行引擎" tabindex="-1"><a class="header-anchor" href="#查询执行引擎" aria-hidden="true">#</a> <strong>查询执行引擎</strong></h2>
<p>在解析和优化阶段，MySQL将生成查询对应的执行计划，MySQL的查询执行引擎根据这个执行计划来完成整个查询。这里执行计划是一个数据结构，而不是和其他的关系型数据库那样生成对应的字节码。</p>
<h2 id="返回结果给客户端" tabindex="-1"><a class="header-anchor" href="#返回结果给客户端" aria-hidden="true">#</a> <strong>返回结果给客户端</strong></h2>
