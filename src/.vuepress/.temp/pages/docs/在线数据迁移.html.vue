...
<figure><img src="https://static001.infoq.cn/resource/image/e0/8a/e043082562e046bef188321d2a97ec8a.png" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
<p>图 1：在线数据迁移步骤示意图</p>
<p>下面以某社交平台粉丝列表从 Mysql 迁移到 HBase 为例子，展开来讲讲每个步骤具体实施、可能的问题及对策。</p>
<p>在迁移之前，根据以往的经验制定了更详细的流程，如图：</p>
<figure><img src="https://static001.infoq.cn/resource/image/61/2c/61bffb2af80ae332bf892ddf74b8b52c.png" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
<p>图 2：粉丝列表迁移到 HBase 工作流程图</p>
<h2 id="上线双写" tabindex="-1"><a class="header-anchor" href="#上线双写" aria-hidden="true">#</a> 上线双写</h2>
<p>编写双写的代码逻辑之前，首先要根据业务规则和性能指标确定 HBase 的表结构和主键设计。</p>
<blockquote>
...
<figure><img src="https://static001.infoq.cn/resource/image/c9/47/c9042f3c2e1658f1383b8b091e602347.png" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
<blockquote>
<p>粉丝列表业务分别使用 HBase 高表模式和宽表模式存储示意图
高表模式的好处在于与 Mysql 类似，各种业务逻辑的实现也比较像，认知和改造成本较低，劣势在于因为 HBase 的实现机制导致单个列表可能被分别存储在多个不同的 Region 里，查询的性能较差。而宽表的优劣势正好与高表相反。在高并发大流量系统中，技术方案很多特性都可以妥协，但唯独性能永远是不能妥协的，所以我们选择宽表模式。</p>
</blockquote>
...
<h2 id="历史数据搬迁" tabindex="-1"><a class="header-anchor" href="#历史数据搬迁" aria-hidden="true">#</a> 历史数据搬迁</h2>
<p>上线双写并校验确认通过后，就可以开始搬迁历史数据了。</p>
...
<p>**历史数据搬迁完成后也需要进行一致性校验。**实际上，建议在搬迁全量数据之前，先搬迁部分数据，并进行一致性校验。部分数据一致性校验通过后，再对全量数据进行搬迁。这种方式可以极大的节约搬迁时间，降低因为搬迁流程或代码不完善导致的延期风险。</p>
<h2 id="切读" tabindex="-1"><a class="header-anchor" href="#切读" aria-hidden="true">#</a> 切读</h2>
<p>全量数据搬迁并校验完成后，即可以进行读请求切换了。通用的切换方式是在代码中埋入开关，通过 Config Service 或类似机制进行切换操作。切换的流程为：Tcpcopy 环境 --&gt; 线上环境 uid 白名单（内部工程师）–&gt; 线上环境百分比灰度 0.01%，1%，10% --&gt; 线上环境全量。tcpcopy 环境用来验证代码在线上环境是否正常，uid 白名单用来验证功能是否正常，百分比灰度用来验证性能和资源压力是否正常，所有验证都通过后，最后才进行全量切换。一般这个过程会持续一周到两周。</p>
<h2 id="清理沉淀" tabindex="-1"><a class="header-anchor" href="#清理沉淀" aria-hidden="true">#</a> 清理沉淀</h2>
<p>切读完成后，整个数据迁移过程可以认为已经完成了。但项目工作并没有完结，旧的逻辑代码清理，旧的配套系统下线，旧资源回收，以及最重要的一个环节：经验教训总结、分享，流程完善，工具通用化改造。</p>
<p>在线数据迁移并不是一项需要高深技术的工作，它更多需要的是对业务逻辑的把控，对操作流程的理解，对新旧系统特性的掌握，以及对细节的敬畏之心。</p>
<blockquote>
<p>参考文章 https://www.infoq.cn/article/online-data-migration-experience</p>
</blockquote>
</div></template>
